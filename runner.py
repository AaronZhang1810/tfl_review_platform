...
Reviews every table in the current edition. For each table: extract (cached by
content hash) → self-check the extraction (Precision guard #1) → the within-table AI
judge (checklist 2.x / 7.2, plus the version items 6.x / 7.3 / 7.4 folded in when a
prior edition exists) → verify pass (Precision guard #2) → dedupe. After the per-table
loop, one cross-output AI judge reconciles numbers across the whole delivery (checklist
3 / 4 / 5 / 7.1 / 8). Finally the deterministic STRUCTURAL checks run (blank pages 1.1,
missing-vs-prior 1.2, numbering gap 1.3). Runs in a background thread; progress is
polled via RUN_PROGRESS.
"""
...
# Every table in the current edition is reviewed. This ceiling is only a runaway
# guard for pathological deliveries; real TLF sets are far below it. Override with
# TLF_MAX_TARGETS if ever needed.
MAX_CURRENT_TARGETS = int(os.environ.get("TLF_MAX_TARGETS", "1000"))

RUN_PROGRESS: dict[int, dict] = {}
_YEAR_RE = re.compile(r"\b(20\d{2})\b")

# Guards RUN_PROGRESS mutation: the per-table pool increments done/conn_errors from
# several threads, and "read, add one, store" is not atomic.
_prog_lock = threading.Lock()

# Review configuration is process-scoped in the current client, while each project
# also has one mutable review snapshot. Letting any two full/single reviews overlap
# can therefore mix model/effort configuration even when they target different
# projects, and can corrupt findings when they target the same one. This deliberately
# process-global lease serializes mutating review runs until all AI call sites take an
# explicit immutable config.
...
def _capture_run_config(model: str | None, effort: str | None) -> dict:
    ...
    configure()+run_config() must be atomic while compatibility globals still
    exist in ai_client.  The returned dict, rather than those globals, is passed
    into every review worker.  ai_client's per-context configuration then keeps
    calls from different projects isolated.
    """
    with _config_lock:
...
# Bump when the JUDGING logic itself changes in a way that should invalidate cached
# per-table judgements even though extraction/model/effort/checklist are unchanged.
_JUDGE_LOGIC_VERSION = "2"


def _checklist_sig(config: dict) -> str:
    """Stable hash of the config parts that change judgements. Editing the checklist (or a
    precision-guard flag) in study_config.json changes this, so cached judgements for every
    table are invalidated and re-judged on the next run."""
...
def _judge_key(cur_hash, prior_hash, model, effort, checklist_sig) -> str:
    """Fingerprint of everything a table's within-table findings depend on. Stored on the
    output after judging; when it still matches, the findings are current and re-judging is
    skipped — which is what makes a run resumable after a crash and fast to re-run."""
...
def _incomplete_by_failure(ex: dict | None) -> bool:
    """True if this stored extraction is missing pages because slice reads FAILED (not
    because the slice cap trimmed them). Such an extraction must never be served from any
    cache — it has to be re-read so the failed pages are retried. A cap-truncation is as
    complete as it will get, so it is NOT flagged here."""
...
def _pick_current_prior(pid: int):
    ...
    When the user marked a main document at creation, exactly one delivery doc keeps
    role='delivery' and the rest are role='prior' — the marked one is current, the
    highest-edition prior is the comparison. With no explicit pick (all role='delivery',
    e.g. a single-file project), the highest edition wins, as before.
    """
    docs = db.query(
...
def _page_texts_for(output: dict) -> list[str]:
    """Per-page text for one output. Extraction is the single most expensive non-API step
    in a run (pdfplumber is ~0.6 s/page), so callers pass the result around rather than
    re-deriving it — see _analyze_table."""
...
def _cached_extraction(output: dict) -> dict | None:
    ...
    If the source file still hashes to what was stored with this extraction, the file is
    byte-identical, so its page text cannot have changed and the extraction is still
    valid. That lets a re-run skip pdfplumber entirely — on an unchanged two-edition
    delivery that is ~18 min of CPU per run spent only to re-derive a cache key.

    Returns None whenever anything is unproven (no stored hash, file changed, file
    unreadable), and the caller falls back to the full text-hash path.
    """
    ...
    # Never serve an extraction left INCOMPLETE by failed page-reads: returning None forces
    # a re-read + slice-retry, and self-heals partial extractions cached by older code
    # (which set src_hash even on a partial read).
...
def _extraction_for(output: dict, pages: list[str] | None = None) -> dict | None:
    ...
    `pages` lets a caller hand in text it already extracted, so one table's text is read
    from the PDF once per run instead of once here and again for the self-check.
    """
    if pages is None:
        pages = _page_texts_for(output)
    full = "\n".join(pages)
    # Hash the FULL text, not a truncated prefix: the hash is the cache-invalidation key,
    # and a 91-page table is ~287k chars, so hashing only the first 60k left ~80% of the
    # table outside the key — a number edited on a later page would not invalidate the
    # cached extraction. The PROMPT stays capped (below); only the key is full-fidelity.
    h = pdftools.content_hash(full)
    text = full[:60000]
    # Read the cache columns from the DB, not the passed dict: a "fresh" run and the
    # per-output re-run NULL these in the DB right before calling us, but the output dict
    # was fetched earlier and still holds the old values — trusting it would wrongly hit
    # the cache and skip re-extraction on unchanged content.
    ...
    if cached and cached.get("extraction_json") and cached.get("content_hash") == h:
        ...
        # The text is unchanged, so a COMPLETE cached extraction still stands — reuse it and
        # stamp src_hash so the next run can skip the PDF read entirely. But if the cached
        # one is incomplete because slices FAILED, do NOT reuse it: fall through and
        # re-extract so those pages are retried. (This is the layer that was silently
        # serving the partial every run — content_hash always matches when the PDF hasn't
        # changed, so without this check the failed pages were never re-attempted.)
    ...
    try:
        ...
        return None
    # A PARTIAL extraction (some page-slices failed to read) is stored so judging can use
    # what we got, but WITHOUT src_hash — that withholds the no-PDF-read fast path so a
    # plain re-run re-reads the file and retries the failed pages instead of reusing the gap.
...
def _analyze_table(o: dict, config: dict, prior_by_label: dict, cfg: dict,
    ...
    Persists this table's within-table findings itself (delete-then-insert under phase
    'within', then stamps output.judge_key) so a completed table is DURABLE — a crash
    leaves it done, and a re-run/resume skips it. Returns the extraction (needed for the
    cross-output bundle) plus a status; it inserts no cross-output/structural findings.
    Exceptions propagate to the caller, which distinguishes a connection blackout (abort)
    from a single-table error (contained).
    """
    if stop.is_set():
        return {"skipped": True}        # a blackout was already detected; don't pile on
    # Re-apply the reviewer's model/effort INSIDE this thread: the server edition holds
    # them in ContextVars, which do not propagate into pool threads.
    ai_client.configure(model=cfg.get("model"), effort=cfg.get("effort"))

    # Read this output's text AT MOST ONCE, and only if something actually needs it.
    # pdfplumber costs ~0.6 s/page and holds the GIL, so it does not overlap with the
    # other pool threads — it was the largest non-API cost in a run.
    ...
    # Precision guard #1: re-read any extracted cell whose count/percent are internally
    # inconsistent, before any judging fires on it. needs_self_check() is a pure detector,
    # so it decides for free whether the source text is needed at all — and on an
    # extraction with no suspect cells, self_check would have made no call anyway.
    ...
    # Prior-edition extraction → the version checklist items (6.x / 7.3 / 7.4) run
    # inside the same within-table judge call.
    pri_ex = None
    ...
    if pri_o:
        ...
        if pri_ex is None:
            # A matched prior means version checks are part of this table's review.
            # Silently continuing without it would turn "comparison unavailable"
            # into a valid zero-finding result and could auto-approve the table.
            raise RuntimeError(
                f"{o['label']}: prior-edition extraction failed; version review incomplete"
            )
        prior_hash = _content_hash_of(pri_o["id"])

    # Continuability: if this table's judge_key still matches, its within-table findings
    # are already current — keep them and skip the (costly) judge calls. This is what a
    # resumed run rides on; a "fresh" run cleared judge_key so it falls through and rejudges.
    ...
    if config.get("dedupe_findings"):
        fs = checks.dedupe(fs)
    # Persist THIS table durably: replace its within-table findings, then stamp judge_key
    # LAST so the table only counts as done once its findings are committed (a crash in
    # between just re-judges it next time). A partial read (some pages failed) is judged on
    # what we have but NOT stamped, so a re-run re-reads and re-judges it — and its failures
    # are surfaced in the run errors so the banner's "check the errors" is real.
...
def _coverage_row(o: dict, ex: dict) -> dict:
    ...
    A table longer than ai_review._MAX_SLICES page-slices is only PARTIALLY extracted -
    rows past the cut never reach the model, so no judge can raise a finding on them. The
    run summary carries this so the UI can say "read 14 / 92 pages" instead of presenting
    a partial extraction as a clean review. Extractions cached before coverage existed
    fall back to the pages that actually produced rows.
    """
    cov = (ex or {}).get("coverage") or {}
    ...
    # Flag on ANY shortfall, not just the slice cap: a slice that errored also leaves
    # pages the judges never saw. `capped` distinguishes the cause so the UI can advise
    # raising TLF_MAX_SLICES only when that is actually the reason.
    if cov:
    ...
    return {"label": o["label"], "pages_read": read, "pages_total": total,
            ...
            "usable": usable,
            # First failure reason (if the shortfall was failed reads, not the cap), so the
            # banner can say WHY a table is partial rather than just that it is.
...
def _blocked_output_ids(pid: int, label_to_id: dict[str, int]) -> set[int]:
    ...
    Cross-output findings have one primary output_id but may compare several tables;
    relying on only the primary would allow the remaining affected tables to be
    auto-approved.
    """
    blocked: set[int] = set()
...
# A cross-output judge call carries EVERY row of every table (see ai_review._compact), which
# on a 70-table delivery is ~445k tokens — more than a 200k-context model takes in one call.
# So the bundle is split and judged in several calls. Budget is in CHARS (~4 chars/token);
# 400k chars ~= 100k tokens, leaving room for the checklist, schema, thinking and output.
...
def _bundle_entry(o: dict, ex: dict) -> dict:
    """One output's contribution to the cross-output judge bundle: groups + footnotes +
    every body row. Table 1 no longer needs a special case — all tables send all rows."""
...
def _family(label: str) -> str:
    ...
    Chunk boundaries must not fall INSIDE a family. Item 7.1 reconciles an AE-overview
    table against its SOC/PT counterpart, and those are siblings (2.2.1 / 2.2.2 / …); if
    they landed in different chunks, no single call would see both and the comparison
...
def _chunk_bundle(bundle: list[dict]) -> list[list[dict]]:
    ...
    Items 3 (pooled), 4 (by-study) and 8 (by-study vs Table 1) all reconcile against
    Table 1, so a chunk without it could not evaluate them. The hub is therefore repeated;
    the resulting duplicate findings are collapsed by checks.dedupe.

    Tables are grouped into families first (see _family) so sibling tables stay in the
    same call. A family larger than the budget still goes out whole rather than being
    split — a too-large call is a visible error, whereas a silently split family is a
    comparison that just never happens.
    """
    if not bundle:
        return []
...
def _select_targets(current_outputs: list[dict]) -> tuple[list[dict], int]:
    ...
    Returns (targets, n_skipped); n_skipped is the outputs NOT reviewed — non-table
    outputs plus any tables beyond the safety ceiling (which real TLF sets never hit)."""
    def is_table(o):
        return ((o.get("output_type") or "").lower() == "table"
                or o["label"].lower().startswith("table"))
    tables = [o for o in current_outputs if is_table(o)]
    # Stable priority: Table 1 first, then by sequence, so the highest-risk
    # reconciliation table is always covered even if the ceiling ever trims.
...
def _n_slices(o: dict) -> int:
    """Extraction calls one output costs: ~one per page, bounded by the slice cap.
    (A page denser than _CHUNK_CHARS splits further, so this is a floor.)"""
...
def count_targets(pid: int) -> dict:
    ...
    The estimate's accuracy hinges on the cache: extraction (per PAGE, ~0.6 s + one API
    call each) is the bulk of a COLD run, but an output that already has `src_hash` takes
    the no-PDF-read fast path and costs nothing. So `extract_calls` counts only the pages
    that will actually be extracted — zero on a warm re-run, the full delivery on a cold
    one. `cached` is how many target tables are already done. Judges re-run regardless, so
    they are counted from `targets`, not here.
    """
...
def measured_throughput(pid: int):
    ...
    Modelled constants chronically under-count real time — the gateway adds per-call
    latency and retries, and effective concurrency is far below MAX_INFLIGHT under rate
    limiting. Actual history is the only reliable signal, so once a run has finished we
    calibrate the estimate against it. Elapsed time is normalised by the run's own target
    count (stored in summary_json), so incremental and fresh runs are comparable. Only
    clean, completed runs with a positive target count are sampled; the median over recent
    runs smooths cold-vs-warm variation. Returns a float, or None when there is no usable
    history."""
    rows = db.query(
...
def start_run(pid: int, kind: str = "incremental",
    ...
    # Resolve everything that defines the run before spawning it.  In particular,
    # do not read mutable client configuration later inside the background thread.
...
def run_structural_checks(pid: int, run_id=None, config=None,
    ...
    """Run the deterministic (non-AI) STRUCTURAL checks for a project and (re)write their
    findings. Idempotent: replaces any existing phase='structural' findings, so it is safe
    to call repeatedly. Needs no API key — used both at project creation (so structural
    issues are visible before any AI review) and inside a full run. Returns the number of
    structural findings written."""
    if current is None:
        current, prior = _pick_current_prior(pid)
    if not current:
        return 0
    ...
    # First collect the entire replacement.  The existing structural snapshot is
    # not touched unless every deterministic check finishes successfully.
...
def _do_run(pid, run_id, kind, current, prior, targets,
    ...
    try:
        ...
        if cfg is None:
            ...
            cfg["checklist_sig"] = _checklist_sig(config)
        # Preflight: one cheap connectivity check before grinding through every output.
        # Only meaningful when the AI is configured and there's work to do. A blackout
        # here aborts the run LOUDLY instead of "finishing" with no findings.
        if targets and ai_client.available():
            ...
            if not ok:
                ...
                return
        # "fresh" = re-read THIS edition from scratch (the reviewer distrusts the current
        # reading). It deliberately does NOT touch the prior edition: last year's PDF is
        # immutable, so re-extracting it costs hundreds of API calls to reproduce byte-
        # identical output. content_hash still guards correctness — if a prior PDF really
        # is replaced, its hash stops matching and it re-extracts on its own.
        # "rebuild" = clear EVERY edition, for when the extraction prompt/schema changed
        # and cached extractions are in the old format.
        # judge_key is cleared alongside the extraction cache so those tables re-judge.
        if kind == "fresh":
            _demote_autoapproved(target_ids)
        ...
        # Do not clear cross/structural findings here.  They are the last complete
        # published snapshot and are atomically replaced only after the new phase
        # finishes successfully. Imported findings are always preserved.
        ...
        # Tables are analyzed CONCURRENTLY. Each table's work is independent (its own
        # extraction + judge), and every step is an API wait, so the run is I/O-bound
        # rather than CPU-bound. ai_client._create caps total in-flight requests, so the
        # pool size here is just how many tables may be in flight — the gateway is
        # protected by that one global semaphore, not by this number.
        ...
        with cf.ThreadPoolExecutor(max_workers=max(1, ai_client.MAX_INFLIGHT)) as pool:
            ...
            for f in cf.as_completed(fut):
                ...
                try:
                    ...
                    # A genuine connectivity blackout must still abort the whole run
                    # loudly. We cannot un-submit work already queued, so signal `stop`
                    # and let the queued tables return immediately; the error is
                    # re-raised once the pool drains.
                    if ai_client.is_connection_error(e):
                        stop.set()
                        results[i] = {"conn_error": e}
                    else:
                        # Any OTHER per-table error is contained so ONE table can't
                        # collapse the run.
                        results[i] = {"error": f"{o['label']}: {e}"}
                with _prog_lock:
                    prog["done"] += 1
                    # Naming the table that just finished reads as random jumping once
                    # tables run concurrently (completion order is arbitrary). A count
                    # is monotonic and actually tells the reviewer where the run is.
                    prog["message"] = f"analyzed {prog['done']}/{len(targets)} tables"

        # Fold the results in TARGET ORDER. Within-table findings were already persisted
        # by each worker (durably, per table); here we only collect extractions for the
        # cross-output bundle and tally what happened.
        n_judged = n_cached = 0
        for i, o in enumerate(targets):
        ...
        if table_phase_complete:
            ...
            cross_ok = True
            # Chunks are independent calls, so run them concurrently under the same global
            # in-flight cap as everything else.
            with cf.ThreadPoolExecutor(max_workers=max(1, ai_client.MAX_INFLIGHT)) as pool:
            ...
            if conn_fatal is not None:
                raise conn_fatal
            if cross_ok:
                # The hub table rides in every chunk, so two chunks can report the same
                # discrepancy — dedupe collapses those before anything is stored.
        ...
        # Same checks that run at project creation; re-run here (idempotent — replaces
        # phase='structural') so a fresh/rebuild run refreshes them against this index.
        ...
        # Total findings now living for this project (within-table were written per worker;
        # cross + structural just above). Counting from the DB is robust to the cache skips.
        n_findings = db.one("SELECT COUNT(*) c FROM finding WHERE project_id=?", (pid,))["c"]
        n_conn = prog.get("conn_errors", 0)
        # AI-unreachable = we had work to do, MOST target extractions failed, and MOST
        # of those failures were connection errors. That's the silent all-clear the UI
        # must flag as NOT a clean review — not a healthy run that happened to find nothing.
        ...
        # A zero-finding result is evidence of cleanliness only after every required
        # stage succeeds with full coverage. Any partial/failed attempt revokes stale
        # system-owned approvals; human statuses are never overwritten.
        n_auto = 0
        if review_complete:
            ...
            # Legacy phase-less AI findings are safe to remove only after the new
            # complete phased snapshot has been published.
...
def run_single_output(oid: int, model: str | None = None,
    ...
    """Re-analyze one output (used by the per-output Re-run button). Replaces that
    output's findings; leaves cross-output findings untouched."""
    ...
    try:
        cfg = _capture_run_config(model, effort)
        cfg["checklist_sig"] = _checklist_sig(config)
        # A single-output pass does not rerun cross-output checks, so it must never
        # leave a stale system-wide clean approval attached to the table.
...
def _replace_phase_findings(pid: int, phase: str, run_id: int | None,
    ...
    The previous phase remains visible if collection/judging fails.  Only after
    every replacement row exists in memory do we delete the old snapshot and
    insert the new one in a single SQLite transaction.
    """
    conn = db.get()
    try:
